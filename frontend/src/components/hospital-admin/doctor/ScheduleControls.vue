    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md">
      ...
      <form @submit.prevent="handleSubmit" class="px-6 py-4 space-y-4">
        ...
        <div>
          ...
          >
          <input
            v-model.number="form.slot_duration_min"
            type="number"
            min="5"
            max="240"
            class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#004795]/30 focus:border-[#004795] transition"
        ...
        <div class="flex items-center gap-3">
          <button
            ...
            :class="[
              ...
              form.is_available ? 'bg-[#004795]' : 'bg-gray-300',
            ]"
          >
            <span
              :class="[
...
/**
 * Normalize any time string to HH:MM (24-hour) for use with <input type="time">.
 * Handles: "08:30", "08:30:00", "08:30 AM", "8:30 AM", "4:30 PM", null/undefined.
 */
function normalizeTime(value, fallback = "") {
  if (!value) return fallback;
  const str = String(value).trim();

  // Already HH:MM or HH:MM:SS (24-hour) — just take first 5 chars
  if (/^\d{1,2}:\d{2}(:\d{2})?$/.test(str)) {
    const [h, m] = str.split(":");
    return String(parseInt(h, 10)).padStart(2, "0") + ":" + m.padStart(2, "0");
  }

  // 12-hour with AM/PM: "8:30 AM", "04:30 PM", etc.
  ...
  (s) => {
    if (s) {
      form.value = {
        ...
        start_time: normalizeTime(s.start_time, "08:30"),
        end_time: normalizeTime(s.end_time, "16:30"),
        lunch_start: normalizeTime(s.lunch_start, "12:00"),
        lunch_end: normalizeTime(s.lunch_end, "13:00"),
