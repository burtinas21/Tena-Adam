      ...
      >
        <div>
          ...
          <p class="text-xs text-gray-400 mt-0.5">
            {{
              isEditing
                ? `Editing ${DAY_LABELS[selectedHour?.day_of_week]}`
                : "Set hours for a day of the week."
            }}
          </p>
        </div>
        <button
      ...
      <form @submit.prevent="handleSubmit" class="px-6 py-5 space-y-4">
        ...
        <div class="flex items-center gap-3">
          <button
            ...
            :class="form.is_holiday ? 'bg-red-500' : 'bg-gray-300'"
            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-300 focus:outline-none"
          >
            <span
              :class="form.is_holiday ? 'translate-x-5' : 'translate-x-0'"
              class="absolute left-0.5 h-5 w-5 rounded-full bg-white shadow-md transform transition-transform duration-300"
            ></span>
          </button>
