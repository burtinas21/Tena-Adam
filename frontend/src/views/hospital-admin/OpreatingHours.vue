<template>
  <main class="flex-1 bg-[#F8FAFC] dark:bg-[#0f172a] p-6 overflow-y-auto font-sans dark:text-slate-200">
    ...
    >
      <div>
        ...
        <p class="text-xs text-gray-500 font-medium mt-0.5">
          Configure your hospital weekly operating schedule.
        </p>
      </div>
      <button
        @click="openCreate"
    ...
    <div v-if="store.loading && store.hours.length === 0" class="space-y-3">
      <div
        v-for="n in 4"
        :key="n"
      ...
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
        ...
        <p class="text-sm text-gray-600 mb-5">
          ...
          <span class="font-semibold text-gray-800">{{
            DAY_LABELS[hourToDelete?.day_of_week]
          }}</span
          >?
        </p>
...
import { useOperatingHourStore } from "../../stores/operatingHourStore";
...
const store = useOperatingHourStore();
...
const hourToDelete = ref(null);
const usedDays = computed(() => store.hours.map((h) => h.day_of_week));
...
function confirmDelete(hour) {
  hourToDelete.value = hour;
