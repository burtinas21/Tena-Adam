  ...
  <main class="flex-1 bg-[#F8FAFC] dark:bg-[#0f172a] p-3 sm:p-5 overflow-y-auto font-sans dark:text-slate-200">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-y-4 mb-6">
      <div>
        ...
        <p class="text-xs text-gray-500 dark:text-slate-400 font-medium mt-0.5">Manage hospital roles and their permission sets.</p>
      </div>
    ...
    <!-- Info notice -->
    <div class="mb-5 flex items-start gap-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 text-blue-700 dark:text-blue-300 text-xs font-medium rounded-lg px-4 py-3">
      <Info class="w-4 h-4 flex-shrink-0 mt-0.5" />
      You can manage permissions for doctors, receptionists, patients and any custom roles within your hospital.
      Deleting system roles and modifying platform-level roles is restricted.
    </div>
    ...
    <!-- Loading skeleton -->
    <div v-if="roleStore.loading && roleStore.roles.length === 0"
    ...
    <Teleport to="body">
      <div v-if="showPermPanel"
        ...
        <div class="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
          ...
          <!-- Auto-save status -->
          <div class="mx-6 mt-3 flex-shrink-0">
            <div v-if="autoSaving"
          ...
          <div class="flex items-center justify-between px-6 pt-3 pb-2 flex-shrink-0">
            ...
            <div class="flex gap-2">
              <button @click="selectAll" class="text-[11px] font-semibold text-[#004795] dark:text-blue-400 hover:underline">Select all</button>
              <span class="text-gray-300 dark:text-slate-600">|</span>
              <button @click="deselectAll" class="text-[11px] font-semibold text-gray-400 dark:text-slate-500 hover:underline">Deselect all</button>
            </div>
          </div>

          <!-- Permission groups (scrollable) -->
          <div class="flex-1 overflow-y-auto px-6 pb-4 space-y-5">
            <!-- No results -->
            ...
            <div v-for="group in filteredGroups" :key="group.module" class="space-y-2">
              <div class="flex items-center justify-between">
                ...
                <button @click="toggleGroup(group)"
                  class="text-[10px] text-[#004795] dark:text-blue-400 hover:underline font-semibold">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                <label v-for="perm in group.permissions" :key="perm.id"
                  :class="[
                    ...
                    perm.restricted
                      ? 'opacity-40 cursor-not-allowed bg-gray-50 dark:bg-slate-700/50 border-gray-100 dark:border-slate-700'
                      : autoSaving ? 'cursor-wait opacity-70' : 'cursor-pointer hover:bg-blue-50/50 dark:hover:bg-blue-900/10',
                    !perm.restricted && checkedPermissions.has(perm.id)
                  ...
                  <input type="checkbox" :value="perm.id" :checked="checkedPermissions.has(perm.id)"
                    :disabled="perm.restricted || autoSaving"
                    @change="!perm.restricted && togglePermission(perm.id)"
                    class="w-3.5 h-3.5 accent-[#004795] cursor-pointer disabled:cursor-not-allowed" />
                  <div class="flex-1 min-w-0 flex items-center gap-1.5">
                    <span class="text-xs text-gray-700 dark:text-slate-300 font-medium">{{ formatPermName(perm.name) }}</span>
                    <span v-if="perm.restricted"
                      class="text-[9px] bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 px-1 py-0.5 rounded font-bold flex-shrink-0">
                      RESTRICTED
                    </span>
                  </div>
                </label>
          ...
          <!-- Footer: close only (auto-save handles persistence) -->
...
import {
  ...
  CheckCircle2, Loader2, X, MoreVertical, Eye, Search, Info,
...
// Permissions hospital admins cannot grant
const RESTRICTED_PERMISSIONS = ['delete_hospitals', 'create_hospitals']
...
const filteredGroups = computed(() => {
  ...
  const groups = permStore.grouped.map(group => ({
    ...group,
    permissions: group.permissions.map(p => ({
      ...p,
      restricted: RESTRICTED_PERMISSIONS.includes(p.name),
    })),
  }))
  if (!q) return groups
  return groups
...
async function selectAll() {
  checkedPermissions.value = new Set(
    permStore.permissions
      .filter(p => !RESTRICTED_PERMISSIONS.includes(p.name))
      .map(p => p.id)
  )
  await persistPermissions()
}
...
function isGroupAllChecked(group) {
  return group.permissions
    ...
    .every(p => checkedPermissions.value.has(p.id))
}

async function toggleGroup(group) {
  const updated = new Set(checkedPermissions.value)
  const nonRestricted = group.permissions.filter(p => !p.restricted)
  if (isGroupAllChecked(group)) {
    nonRestricted.forEach(p => updated.delete(p.id))
  } else {
    nonRestricted.forEach(p => updated.add(p.id))
  }
  checkedPermissions.value = updated
  await persistPermissions()
}
