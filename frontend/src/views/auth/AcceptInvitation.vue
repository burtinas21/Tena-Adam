...
onMounted(async () => {
  ...
  try {
    const res = await authStore.checkInvitation({ token: token.value, email: email.value });
    firstName.value = res.first_name || "";
    state.value     = "ready";
  } catch (err) {
    state.value   = "invalid";
    message.value = err.response?.data?.message || "This invitation link is invalid or has expired.";
  }
});
...
<template>
  ...
  >
    ...
    <header class="text-center mt-4 relative z-10">
      <div class="text-[#004494] font-bold text-2xl tracking-wide flex flex-col items-center gap-0.5">
        <span>Smart Care</span>
        <span class="text-[10px] uppercase font-semibold text-slate-400 tracking-[0.2em]">Tena-Adam</span>
      </div>
    </header>

    <main class="w-full max-w-md my-auto relative z-10">
      <div class="bg-white rounded-xl border border-slate-100 p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">

        <!-- ── Checking ── -->
        <div v-if="state === 'checking'" class="text-center py-8">
          <svg class="animate-spin w-8 h-8 text-[#004494] mx-auto mb-3" fill="none" viewBox="0 0 24 24">
        ...
        <div v-else-if="state === 'invalid'" class="text-center py-6">
          <div class="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-red-50 text-red-500 mb-4">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/>
            </svg>
          </div>
          <h2 class="text-base font-bold text-slate-800 mb-2">Invitation Invalid</h2>
          <p class="text-sm text-slate-500 mb-6">{{ message }}</p>
          <a href="/login" class="text-xs font-bold text-[#004494] hover:text-[#003370]">← Back to Login</a>
        </div>

        <!-- ── Success ── -->
        <div v-else-if="state === 'success'" class="text-center py-6">
          <div class="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-emerald-50 text-emerald-500 mb-4">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
            </svg>
          </div>
          <h2 class="text-base font-bold text-slate-800 mb-2">Account Activated!</h2>
        ...
        <template v-else>
          <div class="text-center mb-6">
            <div class="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-blue-50 text-[#004494] mb-3">
            ...
            <p class="text-xs text-slate-400 max-w-[280px] mx-auto leading-relaxed">
              Create a secure password to activate your hospital admin account.
            </p>
          </div>

          <!-- Email badge -->
          <div class="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 mb-4">
            <svg class="w-4 h-4 text-slate-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
            </svg>
            <span class="text-xs text-slate-600 font-medium truncate">{{ email }}</span>
          </div>

          <!-- Error banner -->
          <div v-if="error" class="mb-4 rounded-lg bg-red-50 border border-red-200 px-3 py-2.5 text-sm text-red-700 flex items-start gap-2">
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <div>
              ...
              <div v-if="password" class="mt-2 space-y-1">
                <div class="flex justify-between text-[11px] font-medium">
                  ...
                  <span :class="strengthLabel(password).color" class="font-bold">{{ strengthLabel(password).text }}</span>
                </div>
                <div class="grid grid-cols-3 gap-1.5">
                  <div v-for="(cls, i) in strengthBars(password)" :key="i" :class="cls" class="h-1 rounded transition-colors duration-300" />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="relative">
                <input
                  ...
                  required
                  class="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#004795]/30 focus:border-[#004795] pr-10"
                  :class="confirmPassword && confirmPassword !== password ? 'border-red-300' : ''"
                />
              ...
              <p v-if="confirmPassword && confirmPassword !== password"
                class="text-[11px] text-red-500 mt-1 font-medium">Passwords do not match.</p>
            </div>
            ...
            <div class="pt-3">
              ...
              >
                <svg v-if="state === 'submitting'" class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/>
                </svg>
          ...
          <div class="text-center mt-4">
            <a href="/login" class="inline-flex items-center gap-1.5 text-xs text-blue-600 font-bold hover:text-blue-700">
              <span>←</span>
    ...
    <footer class="text-[10px] text-slate-400 font-medium flex items-center gap-1 pb-2 relative z-10">
