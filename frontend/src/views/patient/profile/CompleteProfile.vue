  ...
  <main class="flex-1 bg-[#F8FAFC] p-6 overflow-y-auto font-sans">
    <div class="max-w-2xl mx-auto">
      ...
      >
        ...
        <span v-if="isActive">Profile complete — you can book appointments.</span>
        <span v-else>
          Complete your address, occupation and add a primary emergency contact to activate your account.
        </span>
      </div>

      <!-- Loading skeleton -->
      <div v-if="loadingProfile" class="space-y-4">
        <div v-for="n in 3" :key="n" class="h-16 bg-white rounded-xl border border-gray-100 animate-pulse" />
      </div>
      ...
      <template v-else>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-5">
          ...
          <form @submit.prevent="saveProfile" class="space-y-4">
            <div v-if="profileError"
              ...
              <AlertCircle class="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />{{ profileError }}
            </div>
            <div v-if="profileSuccess"
              ...
              <CheckCircle class="w-3.5 h-3.5 flex-shrink-0" />{{ profileSuccess }}
            </div>
            ...
            <div>
              ...
              <textarea v-model="profileForm.address" rows="2" required
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input v-model="profileForm.occupation" type="text" required placeholder="e.g. Teacher"
              ...
              <div>
                ...
                <input v-model="profileForm.national_id" type="text" placeholder="e.g. 123456789"
                  class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#004795]/30 focus:border-[#004795] transition" />
              </div>
            </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <button @click="openContactForm(null)"
          ...
          <div v-if="contactError"
            ...
            <AlertCircle class="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />{{ contactError }}
          </div>
          ...
          <div v-if="!contacts.length" class="py-8 text-center text-gray-400">
            ...
            <p class="text-xs mt-1">Add at least one primary contact to activate your account.</p>
          </div>
          ...
          <div v-else class="divide-y divide-gray-50">
            <div v-for="contact in contacts" :key="contact.id"
              ...
              <div class="flex items-center gap-1 flex-shrink-0">
                <button @click="openContactForm(contact)"
    ...
    <div v-if="showContactForm"
      ...
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div class="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100">
          ...
          <button @click="closeContactForm" class="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 transition">
            <X class="w-4 h-4" />
          </button>
        </div>
        <form @submit.prevent="saveContact" class="px-6 py-4 space-y-4">
          <div v-if="contactFormError"
            class="flex items-start gap-2 bg-red-50 border border-red-200 text-red-700 text-xs font-medium rounded-lg px-3 py-2.5">
            <AlertCircle class="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />{{ contactFormError }}
          </div>
          <div>
            <label class="block text-xs font-semibold text-gray-700 mb-1.5">
              Full Name <span class="text-red-500">*</span>
            </label>
            <input v-model="contactForm.name" type="text" required placeholder="e.g. Abebe Girma"
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input v-model="contactForm.relationship" type="text" required placeholder="e.g. Spouse"
                class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#004795]/30 focus:border-[#004795] transition" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-700 mb-1.5">
              ...
              <input v-model="contactForm.phone" type="text" required placeholder="+251 911 000 000"
                class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#004795]/30 focus:border-[#004795] transition" />
            </div>
          </div>
          <div>
            <label class="block text-xs font-semibold text-gray-700 mb-1.5">Email</label>
            <input v-model="contactForm.email" type="email" placeholder="contact@example.com"
              class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#004795]/30 focus:border-[#004795] transition" />
          </div>
          <div class="flex items-center gap-3">
            <button type="button" @click="contactForm.is_primary = !contactForm.is_primary"
              :class="contactForm.is_primary ? 'bg-[#004795]' : 'bg-gray-200'"
              class="relative w-10 h-5 rounded-full transition-colors duration-200">
              <span :class="contactForm.is_primary ? 'translate-x-5' : 'translate-x-0.5'"
                class="absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform duration-200" />
            </button>
            <span class="text-sm text-gray-700 font-medium">Set as primary contact</span>
          </div>
...
import {
  User, Phone, Plus, Pencil, Trash2, X,
  CheckCircle, AlertCircle, Loader2, PhoneOff,
...
const InfoItem = defineComponent({
  ...
  setup: (p) => () =>
    h("div", {}, [
      h("p", { class: "text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-0.5" }, p.label),
...
async function load() {
  try {
    ...
    profileError.value = err.response?.data?.message || "Failed to load profile.";
  } finally {
...
async function saveProfile() {
  ...
  try {
    ...
    const data = res.data?.data ?? res.data;
    patient.value = data;
    contacts.value = data.emergency_contacts ?? data.emergencyContacts ?? contacts.value;
    profileSuccess.value = "Profile saved successfully!";
    setTimeout(() => { profileSuccess.value = null; }, 4000);
  } catch (err) {
...
function openContactForm(contact) {
  ...
  if (contact) {
    ...
    contactForm.value = { name: "", relationship: "", phone: "", email: "", is_primary: false };
  }
...
async function saveContact() {
  ...
  try {
    ...
    if (editingContact.value) {
      ...
      const res = await api.post("/patient/emergency-contacts", contactForm.value);
...
async function deleteContact(id) {
  ...
  try {
    ...
    contactError.value = err.response?.data?.message || "Failed to delete contact.";
  }
}
</script>
