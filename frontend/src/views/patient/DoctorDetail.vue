      ...
      <template v-else-if="doctor">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-5">
          <div class="flex flex-col sm:flex-row gap-5 items-start">
            <!-- Avatar -->
            <div class="w-24 h-24 rounded-2xl bg-[#004bb5]/10 border border-blue-100 overflow-hidden flex items-center justify-center flex-shrink-0 relative">
              <img v-if="doctor.profile_picture_url && !imgError"
                :src="doctor.profile_picture_url"
                :alt="fullName"
                class="w-full h-full object-cover"
                @error="imgError = true" />
              <span v-else class="text-2xl font-bold text-[#004bb5]">{{ initials }}</span>
              <!-- Online dot -->
              <span class="absolute bottom-1.5 right-1.5 w-3 h-3 rounded-full border-2 border-white"
                :class="doctor.is_telehealth_available ? 'bg-emerald-400' : 'bg-gray-300'" />
            </div>
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1.5 mt-3">
                ...
                <span v-if="doctor.license_number" class="flex items-center gap-2 text-xs text-gray-600">
                  <BadgeCheck class="w-3.5 h-3.5 text-gray-400 flex-shrink-0" />
                  License: {{ doctor.license_number }}
                </span>
                <span v-if="doctor.consultation_fee" class="flex items-center gap-2 text-xs text-gray-600">
              ...
              <div class="flex flex-wrap gap-2 mt-3">
                <span v-if="doctor.is_telehealth_available"
        ...
        <!-- ── Bio ─────────────────────────────────────────────────── -->
        <div v-if="doctor.bio" class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-5">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-5">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <DetailItem label="Specialization"  :value="doctor.department?.name ?? '—'" />
            <DetailItem label="Hospital"        :value="doctor.hospital?.name ?? '—'" />
            <DetailItem label="Experience"      :value="`${doctor.years_experience ?? 0} years`" />
            <DetailItem label="Consultation Fee" :value="doctor.consultation_fee ? `${doctor.consultation_fee} ETB` : '—'" />
            <DetailItem label="License No."     :value="doctor.license_number ?? '—'" />
            <DetailItem label="Practice Since"  :value="practiceStart" />
            <DetailItem label="Telemedicine"    :value="doctor.is_telehealth_available ? 'Available' : 'Not available'" />
          </div>
        </div>
...
import {
  ...
  BadgeCheck, Banknote, User, Info, Languages,
...
const DetailItem = defineComponent({
  ...
  setup: (p) => () =>
    h("div", {}, [
      h("p", { class: "text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5" }, p.label),
...
const practiceStart = computed(() => {
  ...
  if (!d) return "—";
  return new Date(d).toLocaleDateString("en-ET", { year: "numeric", month: "long" });
});
...
async function load() {
  try {
    ...
    error.value = err.response?.data?.message || "Failed to load doctor profile.";
  } finally {
    loading.value = false;
  }
}
