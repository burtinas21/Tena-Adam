  ...
  <div class="bg-[#f0f4fa] dark:bg-[#0f172a] min-h-screen font-sans">
    <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      >
        <div>
          ...
          <p class="text-sm text-gray-500 mt-0.5">
            Showing {{ filtered.length }} available specialist{{
              filtered.length !== 1 ? "s" : ""
            }}
          </p>
        </div>
        <div
          class="flex items-center gap-2 text-sm text-gray-500 self-end sm:self-auto"
        >
      ...
      <div v-if="loading" class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div
          class="lg:col-span-1 bg-white rounded-xl border border-gray-200 h-80 animate-pulse"
        />
        <div class="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-5">
          <div
            v-for="n in 4"
            :key="n"
            class="bg-white rounded-xl border border-gray-200 h-64 animate-pulse"
          />
        </div>
      </div>
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        <!-- ── Filter sidebar ─────────────────────────────────────── -->
        <aside
          class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm lg:sticky lg:top-6"
        >
          ...
          <div class="border-b border-gray-100 pb-4 mb-4">
            ...
            <div class="space-y-2">
              ...
              >
                ...
                <span class="flex-1"
                  >{{ dept.name }}
                  <span class="text-gray-400">({{ dept.count }})</span></span
                >
              </label>
            </div>
            <button
              v-if="departments.length > 3"
              @click="showAllDepts = !showAllDepts"
          ...
          <div class="border-b border-gray-100 pb-4 mb-4">
            ...
            <div class="flex items-center justify-between">
              ...
              <button
                type="button"
                @click="teleOnly = !teleOnly"
                :class="teleOnly ? 'bg-[#004bb5]' : 'bg-gray-300'"
                class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200"
              >
                <span
                  :class="teleOnly ? 'translate-x-6' : 'translate-x-1'"
        ...
        <main class="lg:col-span-3">
          ...
          >
            ...
            <button
              @click="clearNavbarSearch"
              class="text-xs text-blue-500 hover:underline font-semibold ml-3"
            >
              Clear
            </button>
          </div>
          ...
          >
            ...
            <button
              @click="resetFilters"
              class="mt-3 text-xs text-[#004bb5] font-semibold hover:underline"
            >
              Clear filters
            </button>
          </div>
