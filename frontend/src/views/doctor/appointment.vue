  ...
  >
    <div class="max-w-6xl mx-auto">
      ...
      >
        <div>
          ...
          <p class="text-xs text-gray-500 font-medium mt-0.5">
            View, confirm, refer and manage patient appointments.
          </p>
        </div>
      </div>
      ...
      <template v-if="activeTab === 'all'">
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm">
          ...
          <div
            v-if="store.loading && !store.appointments.length"
            class="p-5 space-y-3"
          >
            <div
              v-for="n in 3"
            ...
            <table class="w-full min-w-[600px] text-left text-xs">
              <thead>
                <tr class="border-b border-gray-100">
                  <th
                    class="px-5 py-2.5 text-[10px] font-bold text-gray-400 uppercase tracking-wider"
                  >
                    Patient
                  </th>
                  <th
                  ...
                  <th
                    class="px-5 py-2.5 text-right text-[10px] font-bold text-gray-400 uppercase tracking-wider"
                  >
                    Actions
                  </th>
                </tr>
                ...
                >
                  ...
                  <td class="px-5 py-3">
                    <div class="flex items-center gap-2">
                      <div
                        class="w-8 h-8 rounded-full bg-[#004795]/10 flex items-center justify-center flex-shrink-0"
                      >
                  ...
                  <!-- Reason -->
                  <td class="px-5 py-3 max-w-[150px]">
                  ...
                  <td class="px-5 py-3">
                    <span
                      ...
                      class="inline-flex items-center gap-1 text-[10px] font-medium text-blue-600 bg-blue-50 border border-blue-100 px-1.5 py-0.5 rounded"
                      ><Monitor class="w-2.5 h-2.5" /> Telemedicine</span
                    >
                  ...
                  <td class="px-5 py-3">
                    <button
                      ...
                      @click="openFiles(appt)"
                      class="inline-flex items-center gap-1 text-[10px] font-semibold text-[#004795] bg-blue-50 border border-blue-200 px-2 py-0.5 rounded-full hover:bg-blue-100 transition"
                    >
                      <Paperclip class="w-2.5 h-2.5" />
                      {{ appt.documents.length }} file{{
                        appt.documents.length > 1 ? "s" : ""
                      }}
                    </button>
                  ...
                  <td class="px-5 py-3">
                    ...
                    >
                    <span
                      v-if="appt.referrals?.length"
                      class="ml-1 text-[9px] font-bold text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-full"
                      >Referred</span
                    >
                  </td>
                  <!-- Actions -->
                      ...
                      <Transition name="appt-dropdown">
                        ...
                        >
                          <button
                            ...
                            class="w-full flex items-center gap-2 px-3 py-2 text-xs text-blue-600 hover:bg-blue-50 transition"
                          >
                            <CheckCircle class="w-3.5 h-3.5" /> Confirm
                          </button>
                          <button
                            v-if="appt.status === 'confirmed'"
                            @click="doComplete(appt.id)"
                            :disabled="store.loading"
                            class="w-full flex items-center gap-2 px-3 py-2 text-xs text-emerald-600 hover:bg-emerald-50 transition"
                          >
                          ...
                          <button
                            ...
                            "
                            @click="openReferModal(appt)"
                            class="w-full flex items-center gap-2 px-3 py-2 text-xs text-purple-600 hover:bg-purple-50 transition"
                          >
                            <ArrowRightLeft class="w-3.5 h-3.5" /> Refer Patient
                          </button>
                          <button
                            v-if="
                              ['pending', 'confirmed'].includes(appt.status)
      ...
      <template v-if="activeTab === 'referrals'">
        <div v-if="referralLoading" class="space-y-3">
          <div
            v-for="n in 3"
            :key="n"
        ...
        <div
          v-else-if="!store.incomingReferrals.length"
        ...
        <div v-else class="space-y-4">
          <div
            v-for="ref in store.incomingReferrals"
            :key="ref.id"
            class="bg-white rounded-xl border border-gray-100 shadow-sm p-5"
          >
            <div
            ...
            >
              <div>
                <p class="text-sm font-bold text-gray-800">
                  {{ patientNameFromAppt(ref.appointment) }}
                </p>
                <p class="text-xs text-gray-500 mt-0.5">
                  {{ formatDate(ref.appointment?.scheduled_time) }} ·
                  {{ formatTime(ref.appointment?.scheduled_time) }}
                </p>
                <p class="text-xs text-gray-600 mt-1">
                  Referred by Dr. {{ ref.referredBy?.user?.first_name }}
                  {{ ref.referredBy?.user?.last_name }}
                </p>
                ...
                <div
                  v-if="ref.appointment?.documents?.length"
                  class="flex flex-wrap gap-1.5 mt-2"
                >
                  <button
                    v-for="doc in ref.appointment.documents"
                    :key="doc.id"
              ...
              <div class="flex items-center gap-2 flex-shrink-0">
                <button
                  @click="openReferralResponse(ref, 'accept')"
                  :disabled="store.loading"
                  class="px-4 py-2 text-xs font-bold text-white bg-emerald-500 hover:bg-emerald-600 rounded-lg transition disabled:opacity-50 flex items-center gap-1.5"
                >
                  <CheckCircle class="w-3.5 h-3.5" /> Accept
                </button>
                <button
                  @click="openReferralResponse(ref, 'reject')"
                  :disabled="store.loading"
                  class="px-4 py-2 text-xs font-bold text-white bg-red-500 hover:bg-red-600 rounded-lg transition disabled:opacity-50 flex items-center gap-1.5"
                >
    ...
    >
      ...
      >
        ...
        >
          <div>
            <h3 class="text-sm font-bold text-gray-800">Patient Documents</h3>
            <p class="text-xs text-gray-400 mt-0.5">
              Uploaded at booking by {{ patientName(viewingAppt) }}
            </p>
          </div>
          <button
        ...
        <div class="px-6 py-4 space-y-2 overflow-y-auto flex-1">
          ...
          >
            ...
            <button
              @click="downloadDoc(doc)"
              :disabled="downloading[doc.id]"
              class="ml-3 flex-shrink-0 px-3 py-1.5 text-xs font-semibold text-[#004795] border border-[#004795]/30 rounded-lg hover:bg-[#004795]/5 transition flex items-center gap-1 disabled:opacity-50"
            >
              <Loader2
                v-if="downloading[doc.id]"
    ...
    >
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md flex flex-col">
        ...
        >
          <div>
            <h3 class="text-sm font-bold text-gray-800">Refer Patient</h3>
            <p class="text-xs text-gray-400 mt-0.5">
              Transfer this appointment to another doctor or department
            </p>
          </div>
          <button
            @click="closeReferModal"
            class="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 transition"
          >
            <X class="w-4 h-4" />
          </button>
        ...
        <div class="px-6 py-4 space-y-4">
          ...
          <div>
            ...
            >
            <select
              v-model="referForm.referred_to_doctor_id"
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-700 mb-1.5"
              >Or Refer to Department
              <span class="text-gray-400 font-normal">(optional)</span></label
            >
            <select
              v-model="referForm.referred_to_department_id"
              class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#004795]/30 focus:border-[#004795] transition"
            >
        ...
        >
          ...
          <button
            @click="submitRefer"
            :disabled="!referForm.reason.trim() || referSaving"
            class="px-5 py-2 text-sm font-semibold text-white bg-purple-600 hover:bg-purple-700 rounded-lg transition disabled:opacity-50 flex items-center gap-2"
          >
            <Loader2 v-if="referSaving" class="w-3.5 h-3.5 animate-spin" /> Send
            Referral
          </button>
        </div>
      </div>
    ...
    >
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm flex flex-col">
        ...
        <div class="px-6 py-4 space-y-3">
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-700 mb-1.5"
              >Reason for rejection <span class="text-red-500">*</span></label
            >
            <textarea
              v-model="rejectReason"
              rows="3"
              placeholder="Explain why you cannot accept this referral..."
              class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#004795]/30 focus:border-[#004795] transition resize-none"
            />
          </div>
        </div>
        ...
        >
          ...
          <button
            @click="submitReject"
            :disabled="!rejectReason.trim() || store.loading"
            class="px-5 py-2 text-sm font-semibold text-white bg-red-500 hover:bg-red-600 rounded-lg transition disabled:opacity-50 flex items-center gap-2"
          >
...
function toggleApptMenu(id) {
  openApptMenuId.value = openApptMenuId.value === id ? null : id;
}

const stats = computed(() => [
  {
    label: "Today's",
    value: store.appointments.filter((a) => isToday(a.scheduled_time)).length,
    icon: CalendarDays,
    bg: "bg-blue-50",
    color: "text-blue-500",
  },
  {
    label: "Upcoming",
    value: store.appointments.filter((a) =>
...
function openFiles(appt) {
  viewingAppt.value = appt;
...
const referringAppt = ref(null);
const referForm = ref({
  referred_to_doctor_id: "",
  referred_to_department_id: "",
  reason: "",
});
const referError = ref(null);
const referSaving = ref(false);
...
async function openReferModal(appt) {
  ...
  try {
    ...
    allDoctors.value = (docRes.data?.data ?? docRes.data ?? []).filter(
    ...
    referError.value =
      "Failed to load doctors/departments. " +
...
async function submitRefer() {
  ...
  if (
    !referForm.value.referred_to_doctor_id &&
    !referForm.value.referred_to_department_id
  ) {
    referError.value = "Please select a doctor or department to refer to.";
    return;
  }
  referError.value = null;
  referSaving.value = true;
  try {
    await store.refer(referringAppt.value.id, {
      referred_to_doctor_id: referForm.value.referred_to_doctor_id || null,
      referred_to_department_id:
        referForm.value.referred_to_department_id || null,
...
async function loadIncoming() {
  ...
  try {
    await store.fetchIncomingReferrals();
  } finally {
...
const respondingReferral = ref(null);
...
async function submitReject() {
  ...
  rejectError.value = null;
  try {
    await store.respondReferral(
      respondingReferral.value.id,
      "reject",
      rejectReason.value,
    );
...
<style scoped>
.appt-dropdown-enter-active,
.appt-dropdown-leave-active {
  transition:
    opacity 0.12s,
    transform 0.12s;
}
.appt-dropdown-enter-from,
.appt-dropdown-leave-to {
  opacity: 0;
